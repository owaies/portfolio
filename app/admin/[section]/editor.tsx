...
export default function Editor({ table, columns, rows }: { table: string; columns: string[]; rows: EditableRecord[] }) {
  ...
  return <div>
    ...
    {editing && table === 'projects' ? <ProjectForm editing={editing} onClose={() => setEditing(null)} onSubmit={submitProject} busy={busy}/> : editing && table === 'education' ? <EducationForm editing={editing} onClose={() => setEditing(null)} onSubmit={submitEducation} busy={busy}/> : editing && <div className="admin-modal-backdrop" role="presentation"><div className="admin-modal" role="dialog" aria-modal="true" aria-label={`${editing.id ? 'Edit' : 'Add'} ${table}`}><div className="admin-modal-header"><div><p className="mono text-[10px] uppercase tracking-widest text-cyan-300">CONTENT EDITOR</p><h3 className="mt-1 text-xl font-bold">{editing.id ? 'Edit' : 'Add New'} {table}</h3></div><button type="button" onClick={() => setEditing(null)} className="admin-icon-button" aria-label="Close editor"><X size={19}/></button></div><form action={submit} className="admin-form"><input type="hidden" name="table" value={table}/>{editing.id && <input type="hidden" name="id" value={editing.id}/>} {columns.filter(c => !['id','created_at','updated_at'].includes(c)).map(c => { const value = editing[c]; if (imageFields.has(c) || pdfFields.has(c)) return renderUploadField(c, value); if (booleanFields.has(c)) return <label key={c} className="admin-check"><input type="checkbox" name={c} value="true" defaultChecked={value === true}/><span>{c.replaceAll('_',' ')}</span></label>; if (textAreaFields.has(c)) return <label key={c} className="admin-field sm:col-span-2"><span>{c.replaceAll('_',' ')}</span><textarea name={c} defaultValue={String(value ?? '')} rows={c === 'value' ? 6 : 4}/></label>; return <label key={c} className="admin-field"><span>{c.replaceAll('_',' ')}</span><input name={c} defaultValue={Array.isArray(value) ? value.join(', ') : String(value ?? '')} placeholder={c === 'technologies' ? 'Python, OpenCV, Supabase' : ''}/></label> })}<div className="admin-form-actions sm:col-span-2"><button type="submit" disabled={busy} className="admin-primary">{busy ? 'Uploading & Saving…' : editing.id ? 'Save Changes' : 'Save'}</button><button type="button" onClick={() => setEditing(null)} className="admin-secondary">Cancel</button></div></form></div></div>}
