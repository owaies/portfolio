...
export default function Editor({table,columns,rows}:{table:string;columns:string[];rows:any[]}) {
  ...
  return <div>
    ...
    <div className="space-y-3">
      {rows.map(row => <div key={row.id??row.key} className="glass rounded-2xl p-4"><div className="flex flex-wrap items-start justify-between gap-4"><div className="grid min-w-0 flex-1 gap-2 sm:grid-cols-2">{columns.filter(c=>!['id'].includes(c)).map(c => <div key={c}><div className="mono text-[10px] uppercase tracking-wider text-slate-500">{c}</div><div className="truncate text-sm text-slate-200">{typeof row[c]==='boolean'?String(row[c]):Array.isArray(row[c])?row[c].join(', '):String(row[c]??'')}</div></div>)}</div><div className="flex shrink-0 gap-2"><button onClick={()=>setEditing(row)} className="glass rounded-lg px-3 py-2 text-sm">Edit</button>{row.id&&<button onClick={()=>remove(row.id)} className="rounded-lg border border-red-300/20 px-3 py-2 text-sm text-red-300">Delete</button>}</div></div></div>)}
