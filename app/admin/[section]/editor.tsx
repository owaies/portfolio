...
type EditableRecord = Record<string, any> & { id?: string }
...
export default function Editor({ table, columns, rows }: { table: string; columns: string[]; rows: EditableRecord[] }) {
  ...
  const submitEducation = async (formData: FormData) => {
    setBusy(true)
    setMessage('')
    try {
      await saveRecord(formData)
      setEditing(null)
      setMessage('Education saved successfully. Refreshing…')
      window.location.reload()
    } catch (e) {
      setMessage(e instanceof Error ? e.message : 'Unable to save education.')
    } finally {
      setBusy(false)
    }
  }
  ...
  const renderUploadField = (field: string, value: unknown) => {
    ...
    return <label key={field} className="admin-field sm:col-span-2">
      <span>{field.replaceAll('_', ' ')}</span>
      <div className="admin-upload">
        <input type="file" accept={isPdf ? 'application/pdf,.pdf' : 'image/*'} onChange={e => setFiles(prev => ({ ...prev, [field]: e.target.files?.[0] ?? null }))}/>
        <div className="admin-upload-inner">{isPdf ? <FileText size={22}/> : <ImageIcon size={22}/>}<strong>{selected || (current ? 'Replace uploaded file' : `Click to upload ${isPdf ? 'pdf' : 'image'}`)}</strong><small>{selected || (current ? current : isPdf ? 'PDF files only' : 'JPG, PNG, WEBP and other images')}</small><Upload size={16}/></div>
      </div>
      {current && !selected && <input type="hidden" name={field} value={current}/>}</label>
  }
  ...
  const projectEditor = editing && table === 'projects'
    ? <ProjectForm editing={editing} onClose={() => setEditing(null)} onSubmit={submitProject} busy={busy}/>
    : null

  const educationEditor = editing && table === 'education'
    ? <EducationForm editing={editing} onClose={() => setEditing(null)} onSubmit={submitEducation} busy={busy}/>
    : null

  const genericEditor = editing && table !== 'projects' && table !== 'education'
    ? <div className="admin-modal-backdrop" role="presentation">
        <div className="admin-modal" role="dialog" aria-modal="true" aria-label={`${editing.id ? 'Edit' : 'Add'} ${table}`}>
          <div className="admin-modal-header"><div><p className="mono text-[10px] uppercase tracking-widest text-cyan-300">CONTENT EDITOR</p><h3 className="mt-1 text-xl font-bold">{editing.id ? 'Edit' : 'Add New'} {table}</h3></div><button type="button" onClick={() => setEditing(null)} className="admin-icon-button" aria-label="Close editor"><X size={19}/></button></div>
          <form action={submit} className="admin-form">
            <input type="hidden" name="table" value={table}/>
            {editing.id && <input type="hidden" name="id" value={editing.id}/>} 
            {columns.filter(c => !['id','created_at','updated_at'].includes(c)).map(c => {
              const value = editing[c]
