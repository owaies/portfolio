...
export default function Editor({ table, columns, rows }: { table: string; columns: string[]; rows: EditableRecord[] }) {
  ...
  const submit = async (formData: FormData) => {
    ...
    try { await saveRecord(formData); setMessage('Saved successfully. Refreshing…'); window.location.reload() }
  ...
  return <div>
    ...
    <div className="admin-toolbar">
      ...
      <button onClick={()=>setEditing({})} className="admin-add"><Plus size={16}/> Add</button>
    </div>
    ...
    {editing && <div className="admin-modal-backdrop" role="presentation">
      <div className="admin-modal" role="dialog" aria-modal="true" aria-label={`${editing.id ? 'Edit' : 'Add'} ${table}`}>
        ...
        <form action={submit} className="admin-form">
          ...
          <div className="admin-form-actions sm:col-span-2"><button disabled={busy} className="admin-primary">{busy ? 'Saving…' : editing.id ? 'Save Changes' : 'Save'}</button><button type="button" onClick={()=>setEditing(null)} className="admin-secondary">Cancel</button></div>
        </form>
      </div>
    </div>}
    ...
    <div className="admin-records">
      {filteredRows.map(row => {
        ...
        })}</div><div className="admin-record-actions"><button onClick={()=>setEditing(value)} className="admin-secondary">Edit</button>{value.id && <button onClick={()=>remove(value.id!)} className="admin-danger">Delete</button>}</div></div>
      })}
