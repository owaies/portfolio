...
export default function ProjectForm({ editing, onClose, onSubmit, busy }: Props) {
  ...
  const [forgeColor, setForgeColor] = useState(String(editing.accent_color ?? '#00D4FF').toUpperCase())
  ...
  useEffect(() => {
    if (!selectedImage) { setImagePreview(thumbnail); return }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    formData.set('title', title.trim()); formData.set('slug', editing.slug ?? slugify(title))
    formData.set('detailed_description', description.trim()); formData.set('short_description', description.trim())
    formData.set('tag', tag.trim() || 'Project'); formData.set('deployment_type', deploymentType)
    formData.set('github_url', githubUrl.trim()); formData.set('live_demo_url', deploymentType === 'deployed' ? liveDemoUrl.trim() : '')
    formData.set('icon', icon ?? ''); formData.set('technologies', technologies); formData.set('display_order', displayOrder)
    formData.set('accent_color', forgeColor.toUpperCase())
    try {
      if (selectedImage) {
        const supabase = createClient(); const safeName = selectedImage.name.replace(/[^a-zA-Z0-9._-]/g, '-').replace(/-+/g, '-'); const path = `projects/${crypto.randomUUID()}-${safeName || 'project-image'}`
        const { error } = await supabase.storage.from('project-images').upload(path, selectedImage, { upsert: false, contentType: selectedImage.type || undefined })
        if (error) throw new Error(`Project image upload failed: ${error.message}`)
        const { data } = supabase.storage.from('project-images').getPublicUrl(path); if (!data.publicUrl) throw new Error('Unable to create a public project image URL.')
        formData.set('thumbnail', data.publicUrl)
      } else if (thumbnail) formData.set('thumbnail', thumbnail)
  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center bg-black/80 p-3 backdrop-blur-md">
    <div className="flex max-h-[92vh] w-full max-w-[760px] flex-col overflow-hidden rounded-[30px] border border-blue-400/20 bg-[#020617] shadow-[0_30px_100px_rgba(0,0,0,.65),0_0_50px_rgba(71,233,255,.06)]" role="dialog" aria-modal="true" aria-labelledby="project-editor-title">
      <div className="flex shrink-0 items-center justify-between border-b border-white/[.07] bg-[#020617] px-6 py-5 sm:px-8"><h3 id="project-editor-title" className="text-[28px] font-bold text-white sm:text-[32px]">{editing.id ? 'Edit Project' : 'Add New Projects'}</h3><button type="button" onClick={onClose} disabled={busy} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-white/10 text-slate-400 transition hover:text-white" aria-label="Close project editor"><X size={27}/></button></div>
      <form onSubmit={handleSubmit} noValidate className="min-h-0 overflow-y-auto overscroll-contain px-6 py-6 sm:px-8"><div className="space-y-7">
      ...
      </div><div className="sticky bottom-0 mt-8 flex justify-end gap-3 border-t border-white/[.06] bg-[#020617] pt-5"><button type="button" onClick={onClose} disabled={busy} className="rounded-xl border border-white/10 bg-white/[.02] px-6 py-3 text-base text-slate-400 transition hover:text-white">Cancel</button><button type="submit" disabled={busy} className="inline-flex items-center gap-2 rounded-xl bg-cyan-400 px-7 py-3 text-base font-bold text-slate-950 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"><Save size={19}/>{busy ? 'Saving…' : 'Save'}</button></div></form>
    </div>
  </div>
}
