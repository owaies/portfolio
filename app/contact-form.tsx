...
export default function ContactForm(){
 ...
 async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const formElement=e.currentTarget;const form=new FormData(formElement);setLoading(true);setStatus('');try{const r=await fetch('/api/contact',{method:'POST',body:form});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Unable to send your message.');setStatus('Message sent successfully. I’ll get back to you soon.');formElement.reset()}catch(err){setStatus(err instanceof Error?err.message:'Unable to send your message.')}finally{setLoading(false)}}
