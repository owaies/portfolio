...
const FORGE_STYLE = `
body[data-ui-experience="obsidian-forge"] .forge-cinematic-hero{position:absolute;inset:0;z-index:0;overflow:hidden;pointer-events:none;background:#050607}
...
body[data-ui-experience="obsidian-forge"] .forge-cinematic-grade{position:absolute;inset:0;z-index:1;background:linear-gradient(90deg,rgba(3,4,5,.38),rgba(3,4,5,.12) 44%,rgba(3,4,5,.04) 72%,rgba(3,4,5,.22)),linear-gradient(180deg,rgba(2,3,4,.16),transparent 48%,rgba(2,3,4,.54) 100%)}
...
body[data-ui-experience="obsidian-forge"] .forge-cinematic-copy-veil{position:absolute;z-index:1;left:0;top:12%;bottom:8%;width:min(58%,760px);background:linear-gradient(90deg,rgba(3,4,5,.30),rgba(3,4,5,.08) 68%,transparent)}
...
body[data-ui-experience="obsidian-forge"] .target-hero-inner{z-index:3!important}
body[data-ui-experience="obsidian-forge"] .target-portrait-wrap{display:none!important}
body[data-ui-experience="obsidian-forge"] .target-hero h1{width:auto!important;max-width:9ch!important;font-size:clamp(4.25rem,8vw,8.9rem)!important;line-height:.82!important;color:transparent!important}
body[data-ui-experience="obsidian-forge"] .target-hero h1::before{content:"BUILD\\A INTELLIGENT\\A SOLUTIONS.\\A MOHAMMED"!important;white-space:pre!important;display:block!important;color:#faf8f2!important;text-shadow:0 3px 30px rgba(0,0,0,.5)!important}
body[data-ui-experience="obsidian-forge"] .target-hero h1 > *{visibility:hidden!important}
...
body[data-ui-experience="obsidian-forge"] .forge-cinematic-portrait{left:70%;bottom:-1%;width:min(70vw,430px);height:min(68dvh,620px);transform:translateX(-46%);filter:contrast(1.08) brightness(.9) saturate(.88)}
body[data-ui-experience="obsidian-forge"] .forge-cinematic-rim{left:70%;width:min(70vw,430px);height:min(68dvh,620px);transform:translateX(-46%)}
body[data-ui-experience="obsidian-forge"] .forge-cinematic-copy-veil{top:12%;bottom:8%;width:78%;background:linear-gradient(90deg,rgba(3,4,5,.34),rgba(3,4,5,.08) 72%,transparent)}
...
body[data-ui-experience="obsidian-forge"] .target-hero h1{max-width:9ch!important;font-size:clamp(3rem,12.6vw,5.2rem)!important;line-height:.84!important;letter-spacing:-.055em!important}
...
@media(max-width:390px){body[data-ui-experience="obsidian-forge"] .target-hero h1{font-size:clamp(2.8rem,12.1vw,4.5rem)!important}body[data-ui-experience="obsidian-forge"] .forge-cinematic-portrait{left:72%;width:70vw}}
`
...
export default function ObsidianForgeCinematicHero(){
  ...
  useEffect(()=>{
    ...
    const events=['loadedmetadata','canplay','playing','waiting','stalled','error'] as const
    events.forEach(event=>video.addEventListener(event,()=>report(event)))
    const play=()=>{video.muted=true;void video.play().catch(()=>{})}
    play()
    const onVisibility=()=>{if(document.visibilityState==='visible')play()}
    document.addEventListener('visibilitychange',onVisibility)
    return()=>{events.forEach(event=>video.removeEventListener(event,()=>report(event)));document.removeEventListener('visibilitychange',onVisibility)}
  },[])

  return <>
    <style dangerouslySetInnerHTML={{__html:FORGE_STYLE}}/>
    <div className="forge-cinematic-hero">
      <video ref={videoRef} className="forge-cinematic-video" autoPlay muted loop playsInline preload="auto" poster="/obsidian-forge-environment.svg" aria-hidden="true">
