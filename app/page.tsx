...
export default async function Home() {
  ...
  const [projectResult, skillResult, languageResult, experienceResult, educationResult, certificateResult, resumeResult, profileResult] = await Promise.all([
  ...
  return <main className="target-site">
    ...
    <section id="home" className="target-hero"><div className="target-grid" /><div className="target-hero-inner">
      ...
      <div className="target-hero-copy"><p className="target-eyebrow">› HELLO, WORLD — I&apos;M</p><h1>MOHAMMED<br /><span>OWAIES</span></h1><AnimatedRole /><p className="target-hero-description">{c.hero_subtitle}</p>
        <div className="target-actions"><a className="target-hire" href="#projects"><ArrowUpRight size={12} /> View My Work</a>{resume?.resume_pdf && <a href={`/api/resume/${resume.id}`} target="_blank" rel="noreferrer"><Download size={12} /> Download Resume</a>}<a href={`https://wa.me/${c.whatsapp_number}`} target="_blank" rel="noreferrer" className="target-green"><MessageCircle size={12} /> WhatsApp</a></div>
        <div className="target-stats"><Stat value={c.hero_projects_count} label="PROJECTS" /><Stat value={c.hero_cgpa} label="CURRENT CGPA" /><Stat value={c.hero_certs_count} label="CERTIFICATIONS" /></div>
      </div><div className="target-hero-quote">“In every walk with nature, one receives far more than he seeks.”<small>— John Muir</small></div><a className="target-scroll" href="#about-copy">SCROLL TO EXPLORE <ArrowDown size={12} /></a>
    </div></section>
    ...
    <Section id="projects" eyebrow="// 03 — Projects" title="What I’ve Built" subtitle="Real projects, practical systems, and experiments where intelligence meets software." extra={<a className="target-view-all" href={c.github_url} target="_blank" rel="noreferrer">⌘ View all on GitHub ↗</a>}><div className="target-project-grid">{projects.map((project, index) => <ProjectCard key={project.id} project={project} index={index} />)}</div></Section>
    <Section id="experience" eyebrow="// 04 — Experience & Education" title="My Journey" subtitle="A path of continuous learning, building, and applied problem solving."><div className="target-journey">{experiences.length > 0 && <div className="target-experience-list">{experiences.map((item, index) => <ExperienceCard key={item.id} item={item} index={index} />)}</div>}<div className="target-timeline">{education.map((item, index) => <EducationCard key={item.id} item={item} index={index} cgpa={c.hero_cgpa} />)}</div></div></Section>
    <Section id="certificates" eyebrow="// 05 — Certificates" title="Verified Credentials" subtitle="Continuous learning for a better tomorrow."><div className="target-certificate-grid">{certificates.map((certificate, index) => <CertificateCard key={certificate.id} certificate={certificate} index={index} />)}</div></Section>
    ...
    <Section id="contact" eyebrow="// 08 — Contact" title="Let’s Build Something Together" subtitle="Have a project idea, collaboration, or just want to connect? I’d love to hear from you."><div className="target-contact-grid"><div className="target-contact-list"><ContactCard icon={<Mail size={15} />} label="Email" value={c.contact_email} href={`mailto:${c.contact_email}`} /><ContactCard icon={<Phone size={15} />} label="Phone" value={`+91 ${c.contact_phone}`} href={`tel:+91${c.contact_phone}`} /><ContactCard icon={<MessageCircle size={15} />} label="WhatsApp" value={`WhatsApp · ${c.contact_phone}`} href={`https://wa.me/${c.whatsapp_number}`} /><ContactCard icon={<MapPin size={15} />} label="Location" value={c.contact_location} /><ContactCard icon={<Code2 size={15} />} label="GitHub" value="github.com/owaies" href={c.github_url} /><ContactCard icon={<BriefcaseBusiness size={15} />} label="LinkedIn" value={c.linkedin_url.replace(/^https?:\/\/(www\.)?/, "") } href={c.linkedin_url} /></div><ContactForm /></div></Section>
    <footer className="target-footer"><div>MO<span>.</span> <small>AI/ML ENGINEER</small></div><p>© {new Date().getFullYear()} Mohammed Owaies · Built with precision.</p><div className="target-footer-links"><a href="#home">Home</a><a href="#about-copy">About</a><a href="#projects">Projects</a><a href="#contact">Contact</a></div></footer>
  </main>
}
...
function CertificateCard({ certificate, index }: { certificate: Certificate; index: number }) { const viewHref = `/api/certificate?id=${encodeURIComponent(certificate.id)}`; const downloadHref = `${viewHref}&download=1`; return <article className={`target-certificate-card tone-${index % 5}`}><div className="target-certificate-image">{certificate.thumbnail ? <Image src={certificate.thumbnail} alt={certificate.title} fill sizes="(max-width:700px) 50vw, 330px" className="object-cover" unoptimized /> : <Award size={28} />}</div><div className="target-certificate-meta"><Award size={12} /><span>{certificate.issue_date || "Certificate"}</span></div><h3>{certificate.title}</h3><p>{certificate.issuing_organization}</p><div>{certificate.certificate_pdf && <a href={viewHref} target="_blank" rel="noreferrer"><ExternalLink size={10} /> View PDF</a>}{certificate.certificate_pdf && <a href={downloadHref}><Download size={10} /> Download</a>}</div></article> }
