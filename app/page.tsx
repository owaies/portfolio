...
const fallback: Record<string, string> = {
  ...
  hero_cgpa: "7.52", hero_projects_count: "3+", hero_certs_count: "5+", hero_status: "Open to work",
...
export default async function Home() {
  ...
  return <main className="target-site">
    ...
    <section id="home" className="target-hero"><div className="target-grid" /><div className="target-hero-inner">
      <div className="target-portrait-wrap"><div className="target-portrait-ring target-ring-a" /><div className="target-portrait-ring target-ring-b" /><div className="target-portrait"><Image src={image} alt="Portrait of Mohammed Owaies" fill sizes="180px" className="object-cover" unoptimized priority /></div><div className="target-open"><span /> Open to work</div></div>
      <div className="target-hero-copy"><p className="target-eyebrow">› HELLO, WORLD — I&apos;M</p><h1>MOHAMMED<br /><span>OWAIES</span></h1><p className="target-role">&lt; <span>AI/ML Engineer</span> &nbsp; | /&gt;</p><p className="target-hero-description">{c.hero_subtitle}</p>
      ...
      </div><a className="target-scroll" href="#about">SCROLL <ArrowDown size={12} /></a>
    </div></section>

    <section className="target-section target-about-card-section"><div className="target-about-card"><div className="target-about-icon"><Code2 size={15} /></div><div className="target-about-main"><div className="target-about-top"><strong>{c.about_role}</strong><span>{c.about_company}</span></div><p>{c.about_bio}</p><div className="target-facts"><Fact label="LOCATION" value={c.about_location} /><Fact label="DATE OF BIRTH" value={c.about_dob} /><Fact label="DEGREE" value={c.about_degree} /><Fact label="STATUS" value={c.about_status} /></div></div><div className="target-cgpa"><small>current_cgpa</small><b>{c.hero_cgpa}</b></div></div></section>

    <Section eyebrow="// 01 — About" title={c.about_heading}><div className="target-about-copy"><p>{c.about_text_1}</p><p>{c.about_text_2}</p><div className="target-traits">{["Problem Solver", "Fast Learner", "Team Player", "Detail Oriented", "Curious Mind"].map((trait) => <span key={trait}>{trait}</span>)}</div></div></Section>
    <Section eyebrow="// 02 — Skills" title="Technical Arsenal" subtitle="A curated stack built through hands-on projects, certifications, and relentless practice."><div className="target-skill-groups"><SkillPanel title="Languages" items={languageSkills.length ? languageSkills : skills.slice(0, 5)} accent="cyan" /><SkillPanel title="Databases & Tools" items={dataSkills.length ? dataSkills.slice(0, 5) : skills.slice(5, 10)} accent="purple" /></div><div className="target-tech-stack"><span>// tech_stack</span><div>{techStack.map((tech) => <b key={tech}>{tech}</b>)}</div></div></Section>
    <Section eyebrow="// 03 — Projects" title="What I’ve Built" extra={<a className="target-view-all" href={c.github_url} target="_blank" rel="noreferrer">⌘ View all on GitHub ↗</a>}><div className="target-project-grid">{projects.map((project, index) => <ProjectCard key={project.id} project={project} index={index} />)}</div></Section>
    <Section eyebrow="// 04 — Education" title="Academic Journey"><div className="target-timeline">{education.map((item, index) => <EducationCard key={item.id} item={item} index={index} />)}</div></Section>
    <Section eyebrow="// 05 — Certificates" title="Verified Credentials"><div className="target-certificate-grid">{certificates.map((certificate, index) => <CertificateCard key={certificate.id} certificate={certificate} index={index} />)}</div></Section>
    <Section eyebrow="// 07 — Languages" title="Communication Fluency"><div className="target-language-grid">{languages.map((language, index) => <LanguageCard key={language.id} language={language} index={index} />)}</div></Section>
    <Section eyebrow="// 06 — Resume" title="My Resume">{resume?.resume_pdf ? <div className="target-resume-card"><div className="target-resume-preview">{resume.preview_image ? <Image src={resume.preview_image} alt="Resume preview" fill sizes="150px" className="object-contain" unoptimized /> : <Download size={30} />}</div><h3>Resume</h3><p>Download my full resume to learn more about my education, projects, skills, and certifications.</p><div className="target-resume-actions"><a className="target-hire" href={`/api/resume/${resume.id}`} target="_blank" rel="noreferrer"><Download size={12} /> Download Resume</a><a href={`/api/resume/${resume.id}`} target="_blank" rel="noreferrer"><ExternalLink size={12} /> View PDF</a></div></div> : <div className="target-empty">Resume is not published yet.</div>}</Section>
    <Section eyebrow="// 08 — Contact" title="Let’s Build Something Together"><div className="target-contact-grid"><div className="target-contact-list"><ContactCard icon={<Mail size={15} />} label="Email" value={c.contact_email} href={`mailto:${c.contact_email}`} /><ContactCard icon={<BriefcaseBusiness size={15} />} label="LinkedIn" value="Connect on LinkedIn" href={c.linkedin_url} /><ContactCard icon={<MessageCircle size={15} />} label="WhatsApp" value={c.contact_phone} href={`https://wa.me/${c.whatsapp_number}`} /><ContactCard icon={<MapPin size={15} />} label="Location" value={c.contact_location} /></div><ContactForm /></div></Section>
...
function Section({ eyebrow, title, subtitle, extra, children }: { eyebrow: string; title: string; subtitle?: string; extra?: React.ReactNode; children: React.ReactNode }) {
  const words = title.split(" ")
  return <section className="target-section"><div className="target-section-heading"><p className="target-eyebrow">{eyebrow}</p><h2>{words.map((word, index) => index === words.length - 1 ? <span key={`${word}-${index}`}>{word} </span> : <React.Fragment key={`${word}-${index}`}>{word} </React.Fragment>)}</h2>{subtitle && <p className="target-section-subtitle">{subtitle}</p>}{extra}</div>{children}</section>
}
...
function ContactCard({ icon, label, value, href }: { icon: React.ReactNode; label: string; value: string; href?: string }) { const inner = <><span className="target-contact-icon">{icon}</span><span><small>{label}</small><b>{value}</b></span><ArrowUpRight size={12} /></>; return href ? <a className="target-contact-card" href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined}>{inner}</a> : <div className="target-contact-card">{inner}</div> }
