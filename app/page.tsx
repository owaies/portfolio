...
export default async function Home(){
  ...
  return <main className="grid-bg min-h-screen">
    ...
    <section id="experience" className="container py-24 lg:py-32"><SectionTitle eyebrow="06 / EXPERIENCE" title="Experience"/>{experienceRows.length?<div className="space-y-4">{experienceRows.map(e=><div key={e.id} className="glass rounded-2xl p-6"><p className="mono text-xs text-cyan-300">{e.period}</p><h3 className="mt-2 text-xl font-bold">{e.role}</h3><p className="text-slate-400">{e.company}{e.location?` • ${e.location}`:''}</p><p className="mt-3 leading-7 text-slate-400">{e.description}</p></div>)}</div>:<p className="text-slate-500">No experience entries yet.</p>}</section>

    <section id="resume" className="container py-24 lg:py-32"><SectionTitle eyebrow="07 / RESUME" title="Resume"/><div className="glass rounded-3xl p-7 sm:p-9 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-xl font-bold">{resumeRow?.label||'My resume'}</h3><p className="mt-2 text-slate-400">A concise overview of my education, skills, projects and experience.</p></div>{resumeRow?.resume_pdf?<a href={`/api/resume/${resumeRow.id}`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-cyan-300 px-5 py-3 text-sm font-bold text-[#031015]">Download resume <Download size={16}/></a>:<span className="text-sm text-slate-500">Resume coming soon.</span>}</div></section>

    <section id="languages" className="container py-24 lg:py-32"><SectionTitle eyebrow="08 / LANGUAGES" title="Languages"/><div className="flex flex-wrap gap-3">{languageRows.map(x=><span key={x.id} className="glass rounded-full px-5 py-3 text-sm text-slate-300">{x.name}{x.level?` • ${x.level}`:''}</span>)}</div></section>

    <section id="gallery" className="container py-24 lg:py-32"><SectionTitle eyebrow="09 / GALLERY" title="Gallery"/><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{galleryRows.map(x=><article key={x.id} className="glass overflow-hidden rounded-3xl"><div className="aspect-[4/3] overflow-hidden bg-white/[.03]"><img src={x.image_url} alt={x.caption||'Gallery image'} className="h-full w-full object-cover"/></div>{x.caption&&<p className="p-5 text-sm text-slate-400">{x.caption}</p>}</article>)}</div></section>

    <section id="contact" className="container py-24 lg:py-32"><SectionTitle eyebrow="10 / CONTACT" title="Let&apos;s build something useful."/><div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]"><div className="glass rounded-3xl p-7 sm:p-9"><p className="text-lg leading-8 text-slate-300">Have a project, internship opportunity or collaboration in mind? I&apos;d be happy to hear about it.</p><div className="mt-7 grid gap-3"><a href="mailto:owaies786@gmail.com" className="glass rounded-2xl px-5 py-4 text-sm text-cyan-200">owaies786@gmail.com</a><a href="https://github.com/owaies" target="_blank" rel="noreferrer" className="glass rounded-2xl px-5 py-4 text-sm">github.com/owaies</a><a href="https://wa.me/917619329863" target="_blank" rel="noreferrer" className="glass rounded-2xl px-5 py-4 text-sm text-emerald-300">WhatsApp</a></div></div><ContactForm/></div></section>

    <footer className="container border-t border-white/[.07] py-10 text-center text-sm text-slate-600">© {new Date().getFullYear()} Mohammed Owaies. Built with Next.js, TypeScript and Supabase.</footer>
  </main>
}

function Stat({n,label}:{n:string;label:string}){return <div><div className="text-2xl font-bold text-cyan-200">{n}</div><div className="mono mt-1 text-[9px] uppercase tracking-wider text-slate-600">{label}</div></div>}
function SectionTitle({eyebrow,title}:{eyebrow:string;title:string}){return <div className="mb-10"><p className="mono mb-3 text-xs tracking-[.12em] text-cyan-300">{eyebrow}</p><h2 className="text-4xl font-bold tracking-[-.03em] sm:text-5xl">{title}</h2></div>}
