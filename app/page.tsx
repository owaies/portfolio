...
import { ArrowDown, ArrowUpRight, Award, BriefcaseBusiness, Code2, Download, ExternalLink, GraduationCap, Mail, MapPin, MessageCircle, Phone, Github, Linkedin } from "lucide-react"
...
export default async function Home() {
  ...
  return <main className="target-site">
    ...
    <Section id="contact" eyebrow="// 08 — Contact" title="Let’s Build Something Together"><div className="target-contact-grid"><div className="target-contact-list"><ContactCard icon={<Mail size={15} />} label="Email" value={c.contact_email} href={`mailto:${c.contact_email}`} /><ContactCard icon={<Phone size={15} />} label="Phone" value={`+91 ${c.contact_phone}`} href={`tel:+91${c.contact_phone}`} /><ContactCard icon={<MessageCircle size={15} />} label="WhatsApp" value={`WhatsApp · ${c.contact_phone}`} href={`https://wa.me/${c.whatsapp_number}`} /><ContactCard icon={<MapPin size={15} />} label="Location" value={c.contact_location} /><ContactCard icon={<Github size={15} />} label="GitHub" value="github.com/owaies" href={c.github_url} /><ContactCard icon={<Linkedin size={15} />} label="LinkedIn" value={c.linkedin_url.replace("https://", "www.")} href={c.linkedin_url} /></div><ContactForm /></div></Section>
