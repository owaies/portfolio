...
export default async function Home() {
  ...
  return <main className="target-site">
    ...
    <Section eyebrow="// 04 — Education" title="Academic Journey"><div className="target-timeline">{education.map((item, index) => <EducationCard key={item.id} item={item} index={index} cgpa={c.hero_cgpa} />)}</div></Section>
...
function Section({ eyebrow, title, subtitle, extra, children }: { eyebrow: string; title: string; subtitle?: string; extra?: React.ReactNode; children: React.ReactNode }) { const words = title.split(" "); return <section className="target-section"><div className="target-section-heading"><p className="target-eyebrow">{eyebrow}</p><h2>{words.map((word, index) => index === words.length - 1 ? <span key={`${word}-${index}`}>{word} </span> : <React.Fragment key={`${word}-${index}`}>{word} </React.Fragment>)}</h2>{subtitle && <p className="target-section-subtitle">{subtitle}</p>}{extra}</div>{children}</section> }
