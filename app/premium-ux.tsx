    ...
    <div className="premium-loader" role="status" aria-live="polite" aria-label={`${stages.find(stage => !stage.ready)?.label ?? 'Interface ready'}. ${progress}% of loading stages complete.`}>
      <div className="premium-loader-logo" aria-hidden="true">
        <svg viewBox="0 0 180 190" className="premium-apple-svg">
          <defs>
            <clipPath id="apple-progress-clip">
              <path d="M116 31c-8 7-15 9-24 8-1-10 4-19 11-25 8-7 17-10 26-10 1 10-4 20-13 27ZM91 48c17 0 26 8 34 8 9 0 20-8 33-8 11 0 23 5 30 14-30 17-25 58 5 70-6 14-9 20-18 32-10 14-23 31-39 31-15 0-19-10-36-10-18 0-23 10-38 10-16 0-28-16-38-30C7 141-5 104 8 78c9-18 27-29 46-29 15 0 29 9 37 9Z" />
            </clipPath>
          </defs>
          <path className="premium-apple-track" d="M116 31c-8 7-15 9-24 8-1-10 4-19 11-25 8-7 17-10 26-10 1 10-4 20-13 27ZM91 48c17 0 26 8 34 8 9 0 20-8 33-8 11 0 23 5 30 14-30 17-25 58 5 70-6 14-9 20-18 32-10 14-23 31-39 31-15 0-19-10-36-10-18 0-23 10-38 10-16 0-28-16-38-30C7 141-5 104 8 78c9-18 27-29 46-29 15 0 29 9 37 9Z" />
          <rect className="premium-apple-fill" x="0" y="190" width="180" height="190" transform={`translate(0 ${190 - progress * 1.9})`} clipPath="url(#apple-progress-clip)" />
...
export default function PremiumUX({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        if (entry.isIntersecting) entry.target.classList.add('premium-reveal-visible')
      })
  ...
  useEffect(() => {
    ...
    const setSmooth = () => root.classList.toggle('reduce-motion', reduce.matches)
    setSmooth()
    reduce.addEventListener?.('change', setSmooth)
    return () => reduce.removeEventListener?.('change', setSmooth)
  }, [publicPage])
  ...
  useEffect(() => {
    ...
    setStage(0)
    const markData = window.requestAnimationFrame(() => {
      if (!cancelled) setStage(1)
    })

    const media = Array.from(document.querySelectorAll<HTMLImageElement>('main.grid-bg img'))
    const mediaReady = media.length === 0 || media.every(image => image.complete)
    if (mediaReady) setStage(2)
    else {
      let settled = 0
      const settle = () => {
        settled += 1
        if (settled >= media.length && !cancelled) setStage(2)
      }
      media.forEach(image => {
    ...
    return () => {
      cancelled = true
      window.cancelAnimationFrame(markData)
  ...
  useEffect(() => {
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (image) {
        ...
        setLightbox({ src: image.currentSrc || image.src, alt: image.alt, index: Math.max(0, index) })
      }
      const copy = target?.closest<HTMLElement>('[data-copy-value]')
      if (copy) {
        void navigator.clipboard?.writeText(copy.dataset.copyValue ?? '').then(() => toast(copy.dataset.copyLabel ? `${copy.dataset.copyLabel} copied` : 'Copied', 'success')).catch(() => toast('Copy failed. Please try again.', 'error'))
      }
    }
  ...
  useEffect(() => {
    ...
    const focusable = document.querySelector<HTMLElement>('.premium-lightbox-close')
    focusable?.focus()
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
        ...
        const next = event.key === 'ArrowRight' ? (lightbox.index + 1) % images.length : (lightbox.index - 1 + images.length) % images.length
        setLightbox({ src: images[next].currentSrc || images[next].src, alt: images[next].alt, index: next })
      }
      if (event.key === 'Tab') {
        const dialog = document.querySelector<HTMLElement>('.premium-lightbox')
        const elements = dialog ? Array.from(dialog.querySelectorAll<HTMLElement>('button')) : []
        if (elements.length && (event.shiftKey && document.activeElement === elements[0])) {
          event.preventDefault(); elements[elements.length - 1].focus()
        } else if (elements.length && (!event.shiftKey && document.activeElement === elements[elements.length - 1])) {
  ...
  const copyButton = (label: string, value: string) => (
    <button type="button" className="premium-copy-button" data-copy-value={value} data-copy-label={label} aria-label={`Copy ${label}`}>
      <Copy size={15} /> {label}
    </button>
  )
  ...
  return (
    <>
      ...
      <div className="premium-copy-actions" aria-label="Quick copy actions">
        {copyButton('Email', EMAIL)}
        {copyButton('GitHub URL', GITHUB)}
      </div>

      <button type="button" className={`premium-back-top ${showTop ? 'visible' : ''}`} onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} aria-label="Back to top">
        <ArrowUp size={18} />
      </button>

      <nav className="premium-bottom-nav" aria-label="Mobile section navigation">
        {['home', 'about', 'skills', 'projects', 'education', 'contact'].map(id => (
          <a key={id} href={`#${id}`}>{id === 'home' ? 'Home' : id[0].toUpperCase() + id.slice(1)}</a>
        ))}
      </nav>
