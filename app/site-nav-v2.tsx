...
import UIExperienceSwitcher from "./ui-experience-switcher"
...
export default function SiteNavV2({ activeExperience }: { activeExperience: UIExperienceId }) {
  const pathname = usePathname()
  const [open, setOpen] = useState(false)
  const [compact, setCompact] = useState(false)
  const [obsidian, setObsidian] = useState(activeExperience === "obsidian-forge")
  ...
  useEffect(() => { const update=()=>setObsidian(isUIExperienceId(document.body.dataset.uiExperience) ? document.body.dataset.uiExperience === "obsidian-forge" : activeExperience === "obsidian-forge"); update(); const obs=new MutationObserver(update); obs.observe(document.body,{attributes:true,attributeFilter:["data-ui-experience"]}); return()=>obs.disconnect() },[pathname, activeExperience])
